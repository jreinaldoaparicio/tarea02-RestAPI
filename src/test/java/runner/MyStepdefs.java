...
public class MyStepdefs {
    ...
    public void iSendPOSTRequestApiProjectsJsonWithJson(String method, String url, String jsonBody) {
        ...
        response = FactoryRequest.make(method).send(request);
    }

    @Then("^I expected the response code (\\d+)$")
    public void iExpectedTheResponseCode(int expectedResponseCode) {
        System.out.println("Response Code"+response.getResponseCode());
        Assert.assertEquals("Error!! the response code es diferente ", expectedResponseCode, response.getResponseCode());
    }
    ...
    public void iExpectedTheResponseBodyIsEqual(String expectedResponseBody) throws JSONException {
        ...
        Assert.assertTrue("Error !! the response body es diferente",JsonHelper.areEqualJSON(this.replaceVariables(expectedResponseBody), response.getResponseBody()));
    }
